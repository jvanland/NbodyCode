/* Changed kmax initial from kmax(500) to kmax(1000)
   Increased MAXSTP
   Added DIST_CONTROL
 */
struct Output {
	...
	Output(const Int nsavee) : kmax(1000),nsave(nsavee),count(0),xsave(kmax) {
...
template<class Stepper>
struct Odeint {
	static const Int MAXSTP=400000;
	Doub EPS;
	Int nok;
	Int nbad;
	Int nvar;
...
void Odeint<Stepper>::integrate() {
	...
	for (nstp=0;nstp<MAXSTP;nstp++) {
		...
		if (dense)
			...
			out.save(x,y);
#ifdef DIST_CONTROL
		Doub distsq = (y[0]-y[1])*(y[0]-y[1])+(y[2]-y[3])*(y[2]-y[3])+(y[4]-y[5])*(y[4]-y[5]);
		if (distsq < DIST_CONTROL*DIST_CONTROL || distsq > 1e4) {
		        for (Int i=0;i<nvar;i++) ystart[i]=y[i];
			h=s.hdid;
			cout << "Exited integration because binary separation is " << sqrt(distsq) << "AU." << endl;
			return;
		}
#endif
		if ((x-x2)*(x2-x1) >= 0.0) {
		...
		if (abs(s.hnext) <= hmin) {
		        for (Int i=0;i<nvar;i++) ystart[i]=y[i];
			h=s.hdid;
			cout << "Step size too small in Odeint. h = " << abs(s.hnext) << endl;
			return;
		}
		h=s.hnext;
	}
