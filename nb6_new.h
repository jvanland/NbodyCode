...
SMBH name changes from 1 to 0 when in a pair.
...
struct nbody6 {
  ...
  void operator() (int out, int BHB_NAME) {
    ...
    while(initfile) {
      ...
      initfile.read((char*)first, 7*sizeof(int));
      N_tot = first[1];
	...
	N_obj = N_tot;
      ...
      if (N_out == out) {
	//Initialize vectors to current number of objects
	m = new float[N_tot];
	pos = new float[3*N_tot];
	vel = new float[3*N_tot];
	name = new int[N_tot];
	//Read next 20 values into rec
	initfile.read((char*)rec,20*sizeof(float));
	N_pairs = (int) rec[1];
	...
	//Read masses of objects
      	initfile.read((char*)m,N_tot*sizeof(float));
	//Read position of objects
	initfile.read((char*)pos,3*N_tot*sizeof(float));
	//Read velocities of objects
	initfile.read((char*)vel,3*N_tot*sizeof(float));
	//Read names of objects
	initfile.read((char*)name,N_tot*sizeof(int));
